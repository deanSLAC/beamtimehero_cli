"""Matplotlib figures for the XRS pipeline (energy-loss axis).

Mirrors ``spec_data/plotting.py`` conventions: each function returns
``(fig, summary)`` or ``(None, error)``; ``fig_to_base64`` is reused from there.
"""
